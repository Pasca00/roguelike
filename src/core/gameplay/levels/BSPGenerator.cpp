...
BSPGenerator::BSPGenerator(int h, int w, int depth) : IGenerator(h, w) {
	this->depth = depth;
}
...
void BSPGenerator::generateRoomForNode(TreeNode* node) {
	int x = randomInRange(node->x + 1, node->x + node->w / 4);
	int y = randomInRange(node->y + 1, node->y + node->h / 4);
	int w = node->w - 1;
	int h = node->h - 1;
